...
`ctx` exposes the speaking NPC: `name`, `female`, `role`, `hostile`, `smoker`, `alcoholic`, `polish`, `rnd` (stable per-NPC randoms), plus `pick(list)` and the raw `brain` / `bandit` / `player`. It also exposes **`ctx.you`** — the *player's* status and equipment: flags like `armed`, `injured`, `bleeding`, `infected`; values like `weapon`, `health`, `kills`; and helpers `hasTrait("Brave")`, `wears("hazmat")`, `holding("axe")`. (`armed`/`weapon` also count a weapon slung on your back or in your bag.)
